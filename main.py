...
async def call_llm_for_code(prompt: str, task_id: str, image_parts: list) -> dict:
    ...
    if "captcha solver" in normalized_prompt:
        print("--- [LLM_CALL] Detected captcha solver task — using OCR template ---")
        prompt = (
            "Create a FULLY FUNCTIONAL automated CAPTCHA OCR solver. COPY THIS EXACT STRUCTURE:\n\n"
            "```html\n"
            "<!DOCTYPE html>\n"
            "<html><head>\n"
            ...
            "<div class='mb-6'><img id='captchaImage' class='mx-auto border rounded max-w-full' alt='CAPTCHA'></div>\n"
            ...
            "const imageUrl = params.get('url') || 'sample.png';\n"
            ...
            "  });\n"
            "};\n"
            "img.onerror = () => { status.textContent = 'Image load failed'; result.textContent = 'Error loading: ' + imageUrl; };\n"
            "</script>\n"
            "</body></html>\n"
            "```\n\n"
